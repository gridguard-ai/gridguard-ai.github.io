...
function Footer() {
  return (
    <footer className="relative overflow-hidden" role="contentinfo">
      ...
      <div className="section-container py-12 md:py-16 relative text-soft/80">
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* Brand Column */}
          <div className="lg:col-span-2">
            <a
              href="#"
              className="inline-flex items-center gap-2 text-soft font-bold text-xl hover:text-primary transition-colors"
              aria-label={`${BRAND.name} - Home`}
            >
              <LogoIcon className="w-8 h-8 text-primary" />
              <span className="font-display">{BRAND.name}</span>
            </a>
            <p className="mt-4 text-soft/60 max-w-md leading-relaxed">
              {FOOTER.description}
            </p>
            ...
            <div className="mt-6 flex gap-4">
              {FOOTER.socialLinks.map((social) => (
                <a
                  key={social.label}
                  href={social.href}
                  className="w-10 h-10 bg-dark rounded-full flex items-center justify-center text-soft/60 hover:text-primary hover:bg-dark-light transition-colors"
          ...
          {/* Quick Links */}
          <div>
            <h3 className="text-soft font-semibold mb-4">Quick Links</h3>
            <ul className="space-y-3">
              {NAV_LINKS.map((link) => (
                <li key={link.id}>
                  <a
                    href={link.href}
                    className="text-soft/60 hover:text-primary transition-colors"
                  >
                    {link.label}
                  </a>
                </li>
          ...
          {/* Legal Links */}
          <div>
            <h3 className="text-soft font-semibold mb-4">Legal</h3>
            <ul className="space-y-3">
        ...
        {/* Bottom Bar */}
        <div className="mt-12 pt-8 border-t border-soft/10">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-soft/50 text-sm">
              &copy; {BRAND.year} {BRAND.name}. All rights reserved.
            </p>
            <p className="text-soft/50 text-sm flex items-center gap-2">
              Made with
